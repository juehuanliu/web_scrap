...
def get_email_phone(line):
    try:
        new_urls = deque([line[1]])
    except:
        return ['', '']
    ...
    while len(new_urls) and count < 20:
        ...
        try:
            new_emails = set(
                re.findall(r"[a-z0-9\.\-+_]+@[a-z0-9\.\-+_]+\.(?:com|net|cn|us|edu|gov|cc|org)", response.text, re.I))
...
with open(path,'r') as name_file:
    ...
    with open('listwithemail.csv', 'a', newline='') as new_file:
        ...
        for readlines in name_reader:
            ...
            try:
                email_and_phone = get_email_phone(readlines)
            except:
                continue
            emails, phones = email_and_phone[0],email_and_phone[1]
            emls =''
            for em in emails:
            ...
            if emls != '' or phone != '':
                test_writer.writerow([readlines[0],'tech',readlines[1],emls,phone] )
                new_file.flush()
                print(count_test,[readlines[0],'tech',readlines[1],emls,phone])
